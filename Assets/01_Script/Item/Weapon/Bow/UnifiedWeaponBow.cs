...
///
/// 화살 추적:
/// nockPoint는 활 메시 내부 "시위 본"에 부착됨 — 시위 당김 애니메이션에 따라 nockPoint가 움직임.
/// 화살의 월드 트랜스폼 갱신을 LateUpdate에서 수행해 Animator가 본을 갱신한 뒤의
/// 최종 nockPoint 위치를 읽도록 한다. (Update 타이밍은 본 갱신 전이라 시위 애니메이션이 무시됨.)
/// </summary>
public class UnifiedWeaponBow : NetworkBehaviour, IPlayerWeapon
{
    ...
    [Tooltip("화살이 생성될 위치 (활의 시위 본). 비워두면 자신의 Transform 사용.")]
    ...
    // ------------------------------------------------------------
    // Update: 수명/쿨타임/입력
    // (화살 추적은 LateUpdate에서 — Animator 본 갱신 이후의 nockPoint를 읽기 위해)
    // ------------------------------------------------------------
    private void Update()
    {
    ...
    /// <summary>
    /// 화살이 활시위(nockPoint)를 따라가도록 매 프레임 갱신.
    /// LateUpdate에서 실행해야 Animator가 시위 본을 갱신한 후의 최종 위치를 읽을 수 있다.
    /// Update에서 갱신하면 시위 당김 애니메이션이 반영되지 않은 이전 프레임 위치를 읽게 됨.
    /// </summary>
