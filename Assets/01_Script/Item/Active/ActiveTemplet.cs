...
/*
    ScriptableObject 기반 액티브 아이템 템플릿.

    라이프사이클:
    1) 필드에 놓인 아이템 오브젝트의 SetItem이 이 에셋을 ItemManager.active에 주입
    2) 플레이어 입력 → ItemManager.UseActive() 호출
    3) ItemManager가 this.Activate(owner)를 StartCoroutine으로 실행
        (ScriptableObject는 StartCoroutine 불가 → ItemManager가 대신 돌림)
    4) duration 경과 or 타이머 만료 → OnDeactivate로 원복

    실제 아이템을 만들 때 이 클래스를 상속하되 ApplyEffect / RemoveEffect만 오버라이드

    상태는 ItemManager 쪽에 두거나 owner 파라미터로 넘겨서 관리할 것.
*/
[CreateAssetMenu(menuName = "Item/Active/Template")]
public class ActiveItemTemplate : ScriptableObject, IActive
{
    [Header("아이템 설정")]
